...
class NetworkRepository extends ApiRepository {
  ...
  Future<InterxHeaders> getStatusForUri(Uri uri) async {
    try {
      Response<Map<String, dynamic>> response = await getCustomHttpClient(uri).get(
        '/api/status',
        options: options.copyWith(policy: CachePolicy.refreshForceCache).toOptions(),
      );
      InterxHeaders interxHeaders = InterxHeaders.fromHeaders(response.headers);
      ...
      AppLogger().log(message: 'NetworkRepository: $e');
      rethrow;
    }
  }

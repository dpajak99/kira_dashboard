...
class _SliverTabBarView extends State<SliverTabBarView> {
  ...
  late Map<int, bool> pageVisibilityMap = <int, bool>{
    ...widget.children.asMap().map<int, bool>((int index, _) =>
        MapEntry<int, bool>(index, widget.lazyLoadBool == false)),
  };
  ...
  Widget build(BuildContext context) {
    if (widget.maintainStateBool == false) {
      return widget.children[tabIndex];
    }

    if (widget.lazyLoadBool == true) {
      pageVisibilityMap[tabIndex] = true;
    }
    List<int> visibleKeys = pageVisibilityMap.keys
        .where((int i) => pageVisibilityMap[i] == true)
        .toList();

    final List<Widget> childrenVisibilityWidgets =
        visibleKeys.map<Widget>((int i) {
